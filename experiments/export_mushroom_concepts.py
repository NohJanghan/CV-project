...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export top activating mushroom concept examples.")
    parser.add_argument("--experiment_dir", type=str, default="output/mushroom_toxicity")
    parser.add_argument("--dataset_zip", type=str, default="data/mushrooms/raw/combined-kaggle-mushrooms-dataset.zip")
    ...
    parser.add_argument("--model_id", type=str, default="M3", choices=["M3", "M5"])
...
def open_zip_image(zf: zipfile.ZipFile, member: str, size: int) -> Image.Image:
    try:
        raw = zf.read(member)
...
def main() -> None:
    ...
    df = pd.read_csv(experiment_dir / "dataset1_image_manifest_strict.csv")
    if args.model_id == "M5":
        payload_path = experiment_dir / f"mushroom_medoids_k{args.k}.pt"
        title = "M5 mushroom medoid prototype concepts: top activating examples"
    else:
        payload_path = experiment_dir / f"mushroom_concepts_k{args.k}.pt"
        title = "M3 mushroom learned visual concepts: top activating examples"
    ...
    with zipfile.ZipFile(args.dataset_zip) as zf:
        for row_pos, concept_idx in enumerate(selected):
            ...
            for col_pos, sample_idx in enumerate(ranked):
                ...
                image = open_zip_image(zf, df.iloc[sample_idx]["zip_path"], cell)
                canvas.paste(image, (x0, y0))
                species = str(df.iloc[sample_idx]["species"])
                label = str(df.iloc[sample_idx]["binary_label_strict"])
    ...
    out_path = output_dir / f"{args.model_id}_mushroom_concept_examples.png"
